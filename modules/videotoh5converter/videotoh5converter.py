...
from modules.KTS.cpd_auto import cpd_auto
from modules.networks.CNN import ResNet
import os
# import sys
# sys.path.append('../')
    ...
    def generate_dataset(self):
        for video_idx, video_filename in enumerate(tqdm(self.video_list)):
            ...
            if not os.path.exists(os.path.join(self.frame_root_path, video_basename)):
                print(video_basename, os.path.join(
                    self.frame_root_path, video_basename))
            ...
            for frame_idx in tqdm(range(n_frames-1)):
                ...
                if success:
                    ...
                    cv2.imwrite(os.path.join(self.frame_root_path,
                                video_basename, img_filename), frame)
            ...
            change_points, n_frame_per_seg = self._get_change_points(
                video_feat, n_frames, fps)
            ...
            self.h5_file['video_{}'.format(
                video_idx+1)]['change_points'] = change_points
            self.h5_file['video_{}'.format(
                video_idx+1)]['n_frame_per_seg'] = n_frame_per_seg


if __name__ == "__main__":
    gen = Generate_Dataset(
        './videos/Air_Force_One.mp4', './dataset/summe_dataset.h5')
    gen.generate_dataset()
    gen.h5_file.close()
